...
#include <Servo.h>
//   ============================================================
//   COMBINED v2: LINE FOLLOW → WALL FOLLOW → OBJECT DETECT
//   ============================================================
...
//       → Transition to MODE_OBJECT_DETECT when front sensor reads > 200cm
//         (corridor has ended / opening detected).

//     MODE_OBJECT_DETECT (permanent)
//       → Drives straight at full speed (255).
...
// STATE MACHINE
// ============================================================
enum RobotMode { MODE_LINE_FOLLOW, MODE_WALL_FOLLOW, MODE_OBJECT_DETECT };
...
// --- Recovery constants ---
const unsigned long RECOVERY_STOP_MS     = 400;   // Pause before reversing (ms)
const unsigned long RECOVERY_REVERSE_MS  = 300;   // How long to reverse (ms)

// ============================================================
// WALL FOLLOWING CONSTANTS
// ============================================================
const float          WF_FRONT_NEAR_CM       = 22.0f;  // Corridor-entry threshold (also used for WF corner detection).
// Higher value gives the robot more time to decelerate
// from LF speed before hitting the front wall.
const float          WF_RIGHT_NEAR_CM       = 13.0f;
...
// --- Object detect transition threshold ---
const float          WF_OD_FRONT_OPEN_CM    = 200.0f; // Front sensor reads beyond this = corridor ended.
// Triggers transition to MODE_OBJECT_DETECT.
...
// ============================================================
// WALL FOLLOW: STUCK DETECTION (two-tier, front only)
// First trigger: hard reverse + full-power push forward.
// Subsequent triggers: short soft reverse. Never permanently disabled.
// ============================================================
const unsigned long  US_STUCK_MS            = 3000UL;
const float          US_STUCK_BAND_CM       = 1.5f;
const int            US_STUCK_REVERSE_PWM_1 = 255;   // First-stuck: reverse power
const int            US_STUCK_REVERSE_MS_1  = 500;   // First-stuck: reverse duration
const int            US_STUCK_PUSH_PWM_1    = 255;   // First-stuck: forward push power
const int            US_STUCK_PUSH_MS_1     = 1500;  // First-stuck: forward push duration
const int            US_STUCK_REVERSE_PWM_2 = 200;   // Repeat-stuck: softer reverse power
const int            US_STUCK_REVERSE_MS_2  = 250;   // Repeat-stuck: shorter reverse duration

float         stuckFrontRef    = -1.0f;
unsigned long stuckSinceMs     = 0;
bool          stuckTimerActive = false;
bool          stuckFirstFired  = false;  // true after tier-1 has fired once
...
// ============================================================
// TRANSITION CHECK: WALL FOLLOW → OBJECT DETECT
// Called at the top of the wall-follow block each loop.
// Triggers when the front sensor reads > WF_OD_FRONT_OPEN_CM,
// indicating the corridor has ended and open space is ahead.
// ============================================================
bool checkTransitionToObjectDetect(float f) {
    if (frontValid() && f > WF_OD_FRONT_OPEN_CM) {
        Serial.println(">>> Front sensor >200cm — corridor ended. Switching to OBJECT DETECT <<<");
        currentMode = MODE_OBJECT_DETECT;
        return true;
    }
    return false;
}
...
// ============================================================
// TRANSITION CHECK: LINE FOLLOW → WALL FOLLOW
...
void setup() {
    ...
    Serial.println("=== COMBINED v2: LINE FOLLOW -> WALL FOLLOW -> OBJECT DETECT ===");
    Serial.println("Transition 1: IR off -> coast 300ms straight -> right wall < 20cm = wall follow");
    Serial.println("Transition 2: WF front sensor > 200cm = object detect (full speed straight)");
...
void loop() {
    ...
    if (currentMode == MODE_WALL_FOLLOW) {
        ...
        // Check transition to object detect FIRST — takes priority over all WF steering.
        if (checkTransitionToObjectDetect(f)) return;
        ...
        if (stuckFirstFired) Serial.print("  [STUCK-N armed]");
        else                 Serial.print("  [STUCK-1 armed]");
        Serial.print("  ->  ");
        ...
        return;  // done with wall-follow iteration
    }

    // ---- MODE: OBJECT DETECT ----
    if (currentMode == MODE_OBJECT_DETECT) {
        setMotors(160, 160);
        Serial.println("[OD] Full speed straight.");
        return;
    }
}
